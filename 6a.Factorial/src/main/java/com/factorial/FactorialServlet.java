...
public class FactorialServlet extends HttpServlet {
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            steps.append("Calculation steps:<br>");

            if (number == 0 || number == 1) {
                steps.append(number + "! = 1");
            } else {
                steps.append(number + "! = ");
                ...
                steps.append(" = ").append(factorial);
            }

            out.println("<html><head><title>Factorial Result</title>");
            out.println("<style>.container { margin: 20px; padding: 20px; width: 500px; border: 1px solid #ccc; border-radius: 5px; }</style>");
